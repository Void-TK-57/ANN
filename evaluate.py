...
def main(args):
    ...
    # predict the output
    output = model.predict(x_data).flatten()
    # classes
    clases = ["Atacante", "Defensor"]
    # applt substitution of output
    output = list( [clases[ int(round(value)) ] for value in output] )
    # add to dataframe
    data["Classe"] = output

    # save
    data.to_csv("./data/" + args[3], sep=";", header=True, index=True)
    print("Saved.")
